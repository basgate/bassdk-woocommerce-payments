    ...
    public function init_form_fields() 
    {
        ...
        $this->form_fields = array(
            ...
            'merchant_id'=> array(
                ...
                'type'          => 'text',
                'custom_attributes' => array( 'required' => 'required' ),
                'description'   => __('Based on the selected Environment Mode, copy the relevant Merchant ID for test or production environment available on <a href="'.$basgateDashboardLink.'" target="_blank">Basgate dashboard</a>.', $this->id),
            ),
            'merchant_key' => array(
    ...
    public function blinkCheckoutSend($paramData = array())
    {
        ...
        if (!empty($paramData['amount']) && (int)$paramData['amount'] > 0) {
            ...
            if ($this->getSetting('bankOffer') ==1) {
                $basgateParams["body"]["simplifiedPaymentOffers"]["applyAvailablePromo"]= true;
            }
            // for emi subvention
            if ($this->getSetting('emiSubvention') ==1) {
                $basgateParams["body"]["simplifiedSubvention"]["customerId"]= $paramData['cust_id'];
                $basgateParams["body"]["simplifiedSubvention"]["subventionAmount"]= $paramData['amount'];
                $basgateParams["body"]["simplifiedSubvention"]["selectPlanOnCashierPage"]= true;
                //$basgateParams["body"]["simplifiedSubvention"]["offerDetails"]["offerId"]= 1;
            }
            // for DC EMI
            if ($this->getSetting('dcEmi') ==1) {
                $basgateParams["body"]["userInfo"]["mobile"]= $paramData['cust_mob_no'];
